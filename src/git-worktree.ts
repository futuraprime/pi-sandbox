...
/** Keep Git path metadata reads bounded and fail closed when it is too large. */
  ...
  try {
    ...
    if (descriptor !== undefined) {
      try {
        ...
        // Resolution is deliberately fail-closed and non-throwing.
      }
    }
  }
}
 ...
 *
 * Ordinary repositories, malformed metadata, and filesystem errors all return
 * null. Metadata is reciprocally validated and the result is canonicalized
 * before it is used as a sandbox allowance.
 */
export function resolveGitWorktree(sessionCwd: string): GitWorktreeResolution | null {
  ...
  try {
    ...
    const reciprocalContents = readBoundedFile(join(perWorktreeGitDir, "gitdir"));
